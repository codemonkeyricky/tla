...
As the name suggests, an SPMC lockless queue supports a single producer multiple
consumer usage topology.\newline

In the previously described SPSC, the reader assumes the write index is reserved, and
the writer assumes the read index is reserved. While the relationship between read
and write context remains true in SPMC, the complication is now readers compete
to reserve an index during reading. Some design considerations include:
\begin{itemize}
    \item Readers must have a way to reserve an index to read.
    \item Readers may complete the read in an order different from when they reserve the indices.
    \item Readers can perform the reads independently.
\end{itemize}

A primitive in modern CPU architecture is the compare-and-swap (CAS)
instruction. To use CAS, the issuer needs to specify a memory location, a current
value, and a new value. Only when the value in the memory location matches the
current value, the new value will be written. The instruction ensures this is done
...
will fail. Many of the concurrent algorithms are built on top of this primitive. 
