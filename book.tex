...
\end{equation}
$ENABLED\langle A \rangle$ is defined to be the the predicate that is true iff 
action A is enabled. This can be roughly interpreted as if condition in $A$ is
eventually always true, then A will always eventually happen. To understand what
this means, let's start with an example:\newline

A behaviour is described as a series of state transition:\newline
\begin{equation}
    s_0 \rightarrow s_1 \rightarrow s_2 \rightarrow \dots \rightarrow s_n
\end{equation}

A behaviour can permit self transition:
\begin{equation}
    s_0 \rightarrow s_1 \rightarrow s_1 \rightarrow s_1 \dots
\end{equation}
