...
typically hard to reason about correctness, and tend to have high failure cost
because related issues are difficult to reproduce and hard to debug. Good software
engineering practice suggest we need to implement tests, but tests do not cover
the entire solution space exhaustively.\newline 

\textit{How do we ensure the solution is correct by design?}
...
longer practical, with CPU speed has plateau in the past decade or so. The
...
hardware to software, demanding concurrent software solution to maximize
...
The usual anti-pattern is to keep bandaiding the solution until bugs stop
comming in - but how does anyone know if the solution is actually
\textit{correct by design}? To solve this problem, we must rely on tools to do
the reasoning \textit{for us}.
...
required for action A}. The above translates to: if conditions required for
action A to occur is always true, then action A will eventually occur. In the
example from earlier section without weak fairness defined, the elevator may
\textit{stutter} at floor 1 and never go to floor 2. Weak fairness states that
if the condition of an action is met repeatedly (ie. elevator decides to stay on
1F but but \textit{can} go up), the elevator must \textit{eventually} go
up.\newline
...
Running the spec against TLC passes again. But we want to verify the elevator
eventually goes to the top, not just to 2F. Let's modify the Liveness property
again:\newline
\begin{tla}
Liveness == 
    /\ a = BOTTOM ~> a = TOP
...
(eg. if the elevator is stuck on 1F but can go to 2F, it will). In strong
fairness, the elevator doesn't need to be stuck on 2F to go to 3F. If the
elevator ever makes it to 2F, it \textit{can} go to 3F. To enable strong
...
If we take a closer look at the enabling condition for Up, it only requires
current floor to not equal TOP! This means TLC considers Up to be called if it
is called on \textit{any} floor. If the elevator going between 1F and 2F
indefinitely, TLC assumes strong fairness on \textit{Up} is satisfied, because
it is called going from 1F to 2F, even though the elevator never moves past 2F.
