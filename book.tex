...
\@x{ Spec \.{\defeq}}%
\@x{\@s{8.2} \.{\land}\@s{0.16} Init}%
 \@x{\@s{8.2} \.{\land}\@s{0.16} {\Box} [ ToTwo \.{\lor} ToThree \.{\lor} Down
 ]_{ a}}%
\@x{}\bottombar\@xx{}%
\end{tlatex}

The building has 3 floors and the elevator can go either up or down. The
elevator keeps going up until it's the top floor, or keep going down until it's
the bottom floor. Running TLC against the spec will pass with 3 distinct
state.\newline
...
However, TLC reports a violation with this property:
...
to perpetually stay on 1F, and \textit{never} advances to the next state. This
violates the \textit{Liveness} property. This can be fixed by introduce fairness
description.
...
\end{equation}
$ENABLED\langle A \rangle$ is the TLA+ syntax to represent \textit{conditions
required for step A}. The above translates to: if conditions required for step A
to occur is always true, then step A will eventually occur. In the example from
earlier section without weak fairness defined, the elevator may \textit{stutter}
at floor 1 and never advance to floor 2. Weak fairness states that if the
condition for an action is met repeatedly (ie. every step elevator stays at
floor 1 but \textit{can} go up), the elevator must \textit{eventually} go
up.\newline
...
\@x{\@s{8.2} \.{\land}\@s{0.16} {\WF}_{ a} ( ToTwo )}%
\@x{\@s{8.2} \.{\land}\@s{0.16} {\WF}_{ a} ( ToThree )}%
\end{tlatex}
\newline

Running the spec against TLC passes again.\newline

Let's modify the Liveness property again:\newline
\begin{tla}
Liveness == 
    /\ a = 1 ~> a = 3
\end{tla}
...
In short, TLC identified a case where the elevator is perpetually stuck going
between 1F and 2F, but never go to 3F. Weak fairness is no longer enough,
because the the elevator is not stuck on 2F repeatedly, but stuck going between 
1F and 2F. This is where we need strong fairness.
...
In weak fairness, once the state machine is stuck in a state in perpetuity, the
state machine always transition to a possible next state permitted by the spec
(eg. if the elevator is stuck on 1F but can go to 2F, it will). \newline 

In strong fairness, the elevator doesn't need to be stuck on 2F to go to 3F. If
the elevator ever makes it to 2F, it \textit{can} go to 3F. To enable strong
fairness, we need to update the spec definition like so:\newline
\begin{tla}
Spec ==
  /\ Init
  /\ [][ToTwo \/ ToThree \/ Down]_a
...
\@x{\@s{8.2} \.{\land}\@s{0.16} {\WF}_{ a} ( Down )}%
\@x{\@s{8.2} \.{\land}\@s{0.16} {\WF}_{ a} ( ToTwo )}%
\@x{\@s{8.2} \.{\land}\@s{0.16} {\SF}_{ a} ( ToThree )}%
\end{tlatex}
\newline 

With this change, the spec passes TLC again.

\section{Spec Subtlties}

I initially defined Up and Down symmetrically, but SF on Up didn't have the
expected side effect. Thinking about this more I believe it's because the
condition to Up is a not equal to 3, or a equals to 1 or 2. Strong fairness
trips the action when the condition is true, but in the case of a not equal to
3, it's possible the action takes when a = 1 (but never 2), which fulfills the 
