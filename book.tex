...
\begin{itemize}
    ...
    election
    \item The cluster should have only one leader, but can have mutliple with
    different terms 
...
recovery, and more. These details are omitted for this discussion. Nontheless,
the N nodes in the cluster operate \textit{independently} following the above
hueristics. Hopefully this highlights the complexity around verifying the the
...
\@x{ BecomeLeader ( i ) \.{\defeq}}%
 ...
 term ,\, vote\_granted ,\, vote\_received ,\, vote\_requested {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Candidate ( i ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} state [ i ] \.{=}\@w{Candidate}}%
 \@x{\@s{16.4} \.{\land} \.{\lor} \E\, j \.{\in} Servers \.{:} Campaign ( i
 ,\, j )}%
...
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq}}%
\@x{\@s{16.4} \E\, i \.{\in} Servers \.{:}}%
\@x{\@s{27.72} \.{\lor} Leader ( i )}%
\@x{\@s{27.72} \.{\lor} Candidate ( i )}%
\@x{\@s{27.72} \.{\lor} Follower ( i )}%
\end{tlatex}

\begin{itemize}
    \item A server can be in in one of three states, Servers are defined as a set in this protocol, so \textit{function
