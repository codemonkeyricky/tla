...
Since the propietary low power processor was fairly simple (as far as ASIC
design goes) and only had a handful of instructions, I was able to create an
emulator to verify the microcode prior to deploying it on target. The emulator
tracks the system state on every command execution and confirm every step along
the way none of the system invariants have been violated. This included stuff
such as:
\begin{itemize}
...
problem in the industry where the solution space with very high 
\textit{correctness} requirement, such as lockless/waitfree data structure,
distributed algorithms, OS scheduler, etc. The failure cost to these problems 
tend to be high, because they are possibly extremely difficult to debug or
reproduce. \textit{Why isn't there a generic way of solving this problem?}

\section{The Generalized Problem}

Fast forward to now, as I was learning about distributed algorithms, I found out
about TLA+, a formalized solution to what I was looking for.\newline
...
One slight problem: \textit{human are inherently bad at reasoning things
concurrently}.  Humans are fundamentally single-threaded machines. Reasoning
things that execute in parallel is possible, but difficult. It is hard to
...
modelled using TLA+. The book will also contain a few chapters that discuss
