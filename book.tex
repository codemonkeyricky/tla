...
However, TLC \textit{still} reports the same failure. What's going on?\newline
...
indefinitely, TLC assumes strong fairness on up is satisfied, because Up is
called going from 1F to 2F, even though the elevator never moves past 2F. What
we really want is strong fairness on Up for \textit{every floor}. So if elevator
makes to 2F once, it will eventaully go to 3F. If elevator makes to 3F once, it
will eventaully go to 4F, etc. The following is the change required:\newline
