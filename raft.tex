...
The model checker will run the spec as defined, but due to the exponential growth
of states, it is unlikely to be completed in a reasonable amount of time. We need to
simplify the model and possibly trade off some correctness. Careful
...
The main strategy is to \textit{bound} the state graph. The following describes a
...
However, the term \textit{itself} can grow unbounded. This is a key tenant converging protocols rely on, a monotonically increasing counter. We want to \textit{normalize} the range of terms in the cluster so the minimum value resets
