...
In a distributed system, traffic is hashed and then distributed to different servers
...
cluster. With a traditional hashing algorithm, changing the size of the hash space
...
hash value plus one wraps around to the hash of 0. Servers in a consistent hashing 
cluster take up different ranges in the ring. For a given request, the client 
where the request lands by hashing the request first, then walks the ring clockwise 
