...
\textit{Unlock} represents when a running task release the lock. If there are no
blocked tasks, the running task carries on as before. If there are blocked
tasks, they are inserted to the beginning of the ready queue (to maintain the 
lock request ordering) and the running task is forced to give up the CPU and
inserted at the end of the ready queue. 
