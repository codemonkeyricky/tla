...
\textit{Unlock} represents when a running task release the lock. If there are tasks 
in blocked queue, now they may be scheduled. 
If the task attempts to acquire the global lock while the lock is taken,
it'll be moved to the blocked\_q using \textit{Append}. The task then joins the queue of tasks that
