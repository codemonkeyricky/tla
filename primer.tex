...
Note that the formula are \textit{state descriptions}, not \textit{assignment}.
...
and \textit{59 = minute} are equivalent in TLA+ spec. 
...
When hour or minute falls outside of the specified range, the model checker will
report failure.
...
the spec is allowed to \textit{stutter}, or \textit{not transition} to a next
state. This by definition fails liveness property check as the model checker is
...
and other tools, please see \cite{toolbox}.\newline

After the model checker is installed, we need two things to verify the spec:
\begin{itemize}
    \item clock.tla - spec 
