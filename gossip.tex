...
\begin{itemize}
    ...
    \item Each node has a ready status.
    \item Each node caches the version of all other nodes.
    \item A node will randomly initiate gossip with another node.
    \item Only a ready node can initiate gossip. 
    \item Nodes can restart randomly, restart clears the node's version cache of
    all other nodes and puts the node into not ready.
    \item A restarted node becomes ready again after a ready node gossip with it.
    \item A ready node can bump its version.
\end{itemize}
